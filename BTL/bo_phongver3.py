...
# ==========================================
st.set_page_config(page_title="Digital Twin Park V5 (Animation)", layout="wide")
...
# 1. INPUT MODULE
# ==========================================
st.title("🔥 Digital Twin V5: Visual Simulation")
st.markdown("---")
...
with st.sidebar:
    ...
    # Giảm mặc định xuống để demo animation cho mượt, user có thể tăng lên
...
with col_main1:
    st.info("💡 Tọa độ X, Y dùng để vẽ mô phỏng (X: 0-800, Y: 0-600). Cổng vào mặc định ở (400, 580).")
    # Dữ liệu mặc định ĐÃ CẬP NHẬT TỌA ĐỘ GIẢ LẬP
    default_nodes = [
    ...
    # Cần thêm các cột ẩn về kỹ thuật vào sau khi user edit
...
with col_main2:
    ...
    default_tours = [
        {"Giờ đến": time(9, 0), "Số lượng": 20, "Loại đoàn": "Học sinh"}, # Giảm số lượng để demo mượt
...
class ServiceNode:
    def __init__(self, env, name, config):
        ...
        # Mặc định các thông số nếu thiếu
        ...
        # Các chỉ số giả định nếu chưa có trong bảng
        self.rebuy_prob = 0.1 
...
class DigitalTwinPark:
    def __init__(self, env, nodes_config):
        self.env = env
        self.nodes = {}
        self.node_coords = {} # Map tên -> (x, y)
        self.movement_log = [] # NHẬT KÝ DI CHUYỂN CHO ANIMATION
        
        # Cổng vào mặc định
        self.gate_pos = (400, 580)
...
def visitor_journey(env, visitor_id, park, is_combo, entry_time):
    # --- ĐỊNH NGHĨA LOẠI KHÁCH CHO ANIMATION ---
    # 1: Lẻ (Xanh lá), 2: Combo (Xanh dương), 3: Đoàn (Vàng)
    v_type = 2 if is_combo else 1
    if "Tour" in visitor_id: v_type = 3
    
    # Vị trí bắt đầu: Cổng
    current_x, current_y = park.gate_pos
    ...
    while env.now < leave_time and env.now < TOTAL_MINUTES:
        ...
        # -- GHI LOG: TẠI TRẠM (STAYING) --
        arrival_ts = env.now
        
        # Vào khu vực
        if len(target_node.resource.queue) < target_node.queue_cap:
            with target_node.resource.request() as req:
                yield req # Chờ xếp hàng
                yield env.timeout(target_node.service_time) # Chơi/Ăn
    ...
    # Rời công viên (Về lại cổng để biến mất)
    park.movement_log.append({
        "id": visitor_id, "type": v_type,
        "start": env.now, "end": env.now + 10,
        "x1": current_x, "y1": current_y,
        "x2": park.gate_pos[0], "y2": park.gate_pos[1]
    })
    ...
    <body>
        ...
        <script>
            ...
            function draw() {{
                ...
                ctx.fillRect(380, 580, 40, 5);
                ctx.fillText("CỔNG VÀO", 400, 595);
...
if st.button("🚀 CHẠY MÔ PHỎNG (VISUAL + DATA)", type="primary"):
    ...
    st.markdown("### 🎥 Mô phỏng Trực quan (Real-time Playback)")
    st.caption("Các chấm di chuyển đại diện cho từng khách hàng. Bạn có thể chỉnh tốc độ tua nhanh/chậm.")
    render_animation(park.movement_log, edited_nodes_df, OPEN_TIME.hour)
    
    st.markdown("---")
    
    # --- TABS: ANALYTICS ---
    tab_map, tab_fin = st.tabs(["🔥 Heatmap Thống kê", "💰 Doanh thu"])
    ...
    with tab_fin:
        ...
        c2.metric("Vé Cổng", f"{park.gate_revenue:,.0f} VNĐ")
        c3.metric("Dịch vụ", f"{service_revenue:,.0f} VNĐ")